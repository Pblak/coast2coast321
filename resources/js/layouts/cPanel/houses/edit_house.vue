                    ...
                    <div class=" flex gap-6 w-fit items-center">
                        <template v-for="(image,key) in images " :key="key">
                            <div
                                ...
                                :style="!image.file?'background-image:url(\''+VITE_APP_URL+'/storage/'+image.result+'\')'
...
export default {
    data() {
        const images = ref([])
        const {storeHouse, errors, house_status, getHouse, house, update_house} = use_house()
        return {
            images,
            errors,
            house_status,
    ...
    mounted() {
       ...
       let res = this.$route.meta.data
            ...
            this.formData = {
                ...
                listed_for: res.specification.listed_for,
                price: res.specification.price,
                status: res.status,
                property_type: res.specification.property_type,
                surfaceArea: {
                    lot_size: res.specification.surfaceArea.lot_size,
                    building_size: res.specification.surfaceArea.building_size
                },
                description: res.specification.description,
                measurement_unit: res.specification.measurement_unit,
                floors: res.specification.floors
            }
    },
    methods: {
        handleMap() {
        ...
        deleteImage(key) {
            if (this.formData.selectedImage !== key) {
                if (!this.images[key].file){
                    this.deletedImages[key] = this.images[key]
                }
        ...
        save() {
            ...
            for (const k in this.images) {
                if (this.images[k].file) {
                    fd.append('image_' + k, this.images[k].file)
                }
            }
            fd.append('imagesDeleted', JSON.stringify(this.deletedImages))
        ...
        deleteFloor(key) {
            this.formData.floors.deleted = true
        },
        addImage() {
